                  ...
                  Column(
                    children: [
                      MyButton(
                        ...
                        onPressed: () async => await vmContract.signIn('Google', () async => await navigate(context, const StartupView())),
                      ),
                      MyButton(
                        size: Sizes.large,
                        label: const Text('Continue with Facebook'),
                        icon: const FaIcon(FontAwesomeIcons.facebook),
                        onPressed: () async => await vmContract.signIn('Facebook',() async => await navigate(context, const StartupView())),
                      ),
                      MyButton(
                        size: Sizes.large,
                        label: const Text('Continue with Yahoo!'),
                        icon: const FaIcon(FontAwesomeIcons.yahoo),
                        onPressed: () async => await vmContract.signIn('Yahoo',() async => await navigate(context, const StartupView())),
                      ),
                    ],
                  ),
              ],
